	...
	this.each(function(){
	  ...
	  var fileObj = {
				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			l.alert('文件'+thisFile.name+'大小超出限制！');
				  			continue;
				  		}
				  		//arr.push(thisFile);	
						if(option.fileTypeExts=="*"||$.inArray(thisFile.name.split('.').pop(),typeArray)>=0){
							arr.push(thisFile);	
						}else{
							l.alert('文件'+thisFile.name+'类型不允许！');
						} 
					}	
				}
			  return arr;  	
		  ...
		  onProgress: function(file, loaded, total) {
				...
				option.onProgress&&option.onProgress(file,percent);
	  		},		//文件上传进度
		  ...
		  funDealFiles: function(files) {
			  var fileCount =$('#uploadDiv .odUploadList li').length;//队列中已经有的文件个数
			  for(var i=0,len=files.length;i<len;i++){
