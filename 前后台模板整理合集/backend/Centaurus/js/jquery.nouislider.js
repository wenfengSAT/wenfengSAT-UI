...
function setHandle(handle,to,forgive){var nui=handle.data('nui').options,handles=handle.data('nui').base.data(clsList[12])// Get some settings from the handle;,style=handle.data('nui').style,hLimit;if(!$.isNumeric(to)){return false;}
...
function store(handle,S){var i=handle.data('nui').number;if(S.to[i]instanceof $){return S.to[i].data({target:handle.data('nui').target,handle:handle})// Attach a change event to the supplied jQuery object,.on('change'+namespace+' blur'+namespace,function(){var arr=[null,null];arr[i]=$(this).val();$(this).data('target').val(arr,{trusted:false});})// Triggering the 'set' callback should not occur on the 'blur'.on('change'+namespace,function(){call($(this).data('handle').data('nui').options.set,$(this).data('target'));});}
...
var base=event.pass.base,style=base.data('style')// Subtract the initial movement from the current event,,proposal=event.x- event.pass.startEvent.x,baseSize=style==='left'?base.width():base.height();if(style==='top'){proposal=event.y- event.pass.startEvent.y;}
